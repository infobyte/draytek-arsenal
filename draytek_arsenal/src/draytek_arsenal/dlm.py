...
class DLM():
    ...
    key1 = b"\x67\x56\x67\x23\x12\x54"
    key2 = b"VR9KEY"
    ...
    def restore(self, data):
        ...
        if main_id_0 != 0:
            print("Main ID 0: 0x{:x}".format(main_id_0))
            exit(1)
        if main_id_2 != 1:
            print("Main ID 2: 0x{:x}".format(main_id_2))
        if main_id_3 != 0xffff:
            print("Main ID 3: 0x{:x}".format(main_id_3))
            exit(1)
        if signature != 0x12345678:
            print("Signature: 0x{:x}".format(signature))
            exit(1)
        if dlm_data_len != size - 0x34:
            print("dlm data length error")
            exit(1)
        key = self.generate_digest(self.key1, self.key2)
    ...
    def pack(self, data):
        lz4 = Lz4()
        digest = self.generate_digest(data, self.key2)
